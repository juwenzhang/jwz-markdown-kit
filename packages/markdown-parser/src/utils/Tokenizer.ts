...
export const isAtEnd = (state: ParserState)
: boolean => {
  return state.position >= state.input.length;
}

export const getCurrentChar = (state: ParserState)
: string | undefined => {
  return state.input[state.position];
}

export const advance = (state: ParserState, count: number = 1)
: ParserState  => {
  return {
    ...state,
...
export const consumeUntil = (state: ParserState, stopChar: string)
: [string, ParserState] => {
  let result = '';
  let newState = state;
  while (
...
export const consumeWhile = (state: ParserState, condition: (char: string) => boolean)
: [string, ParserState] => {
  let result = '';
  let newState = state;
...
export const parseHeading = (state: ParserState)
: [Token, ParserState] => {
  let level = 0;
  let newState = state;
...
export const parseCodeBlock = (state: ParserState)
: [Token, ParserState] => {
...
export const parseList = (state: ParserState)
: [Token, ParserState] => {
  ...
  while (!isAtEnd(newState)) {
    ...
    const [, stateAfterIndent] = consumeWhile(newState, char => char === ' ' || char === '\t');
    newState = stateAfterIndent;

    const bulletPattern = ordered ? /^\d+\.\s/ : /^[*-]\s/;
    const bulletMatch = newState.input.slice(newState.position).match(bulletPattern);
...
export const parseParagraph = (state: ParserState)
: [Token, ParserState] => {
...
export const tokenize = (input: string): Token[] => {
  ...
  while (!isAtEnd(state)) {
    ...
    if (char === '`' && state.input.slice(state.position, state.position + 3) === '```') {
